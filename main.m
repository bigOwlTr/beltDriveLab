...
plot(data90{:,3}, predict(mdl90, data90{:,3}), 'b:', 'LineWidth', 1, ...
    'DisplayName', '90° Linear Fit');
plot(data180{:,3}, predict(mdl180, data180{:,3}), 'r:', 'LineWidth', 1, ...
    'DisplayName', '180° Linear Fit');
plot(data270{:,3}, predict(mdl270, data270{:,3}), 'g:', 'LineWidth', 1, ...
    'DisplayName', '270° Linear Fit');
plot(data360{:,3}, predict(mdl360, data360{:,3}), 'k:', 'LineWidth', 1, ...
...
text(angles_rad(2), mubeta_values(2), equation_text, 'FontSize', 12, ...
...
xlim([0, 2*pi]);
ylim([min(mubeta_values)-1, max(mubeta_values)+1]);
